   ...
   <!-- Gráfico de vendas -->
    <div style="width: auto">
        <div style="display:flex; flex-direction: row; justify-content: space-between; width: 60%; margin: 0 auto">
            <strong style="margin-left: 15px">{{ $estatisticas->contadormesinicio }}</strong>
            <strong>{{ $estatisticas->contadormespassado }}</strong>
            <strong>{{ $estatisticas->contadormesatual }}</strong>
        </div>
        <a href="{{ getenv('APP_URL') }}/vendas">
            <div class="col-md-6">
                <!-- Bar chart -->
    ...
    <div class="container" style="width: 100%; height: 40%; display: flex; flex-direction: column">
        <a href="{{ getenv('APP_URL') }}/pagamentos">
...
<script type="text/javascript">
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            labels: ["{{ $estatisticas->extmesinicio }}", "{{ $estatisticas->extmespassado }}", "{{ $estatisticas->extmeshoje }}"],
            datasets: [{
                label: ['Quantidade de Vendas'],
                backgroundColor: '#009DAE',
                borderColor: '#009DAE',
                data: [{{ $estatisticas->contadormesinicio }}, {{ $estatisticas->contadormespassado }}, {{ $estatisticas->contadormesatual }}],
            }]
        },
